...
protected:
    ...
    */
    void onImGuiDisplay() override
    {
        ...
        {   
            printf("loading fg %s\n", RES_PATH"/example_fg.svg");
            cutoff_knob = std::make_unique<ImGuiKnobsSVG::Knob>(RES_PATH"/example_fg.svg", ImGuiKnobVariant_Stepped, &kCutoff, 0.0f, 1.0f, 100);
            printf("loading bg %s\n", RES_PATH"/example_bg.svg");
            cutoff_knob->setBg(RES_PATH"/example_bg.svg");
        }
        ...
        if (ImGui::Begin("Simple gain", nullptr, ImGuiWindowFlags_NoResize))
        {
            ...
            if (ImGuiKnobs::Knob("CUT OFF FREQ", &kCutoff, 0.0f, 1.0f, 0.005f, "%.3f", ImGuiKnobVariant_Stepped, 100, 0, 11)) {
